  ...
  confirm(){
    ...
    for (let index = 0; index < this.data.wList.length; index++) {
      ...
      if(element.flag==0){
        ...
        var temp2 = (element.period+1)/2;
        var temp3 = element.flag-1;
        prevPage.setData({
          courseArrangements:prevPage.data.courseArrangements+temp+' '+temp2+' '+temp3+','
        })
        i++;
      }
    }
    const tmp = prevPage.data.courseArrangements
    prevPage.setData({
      courseArrangements:tmp.slice(0,tmp.length-1)
    })
    console.log(prevPage.data.courseArrangements)
    wx.navigateBack({
      delta: 1
    })
  }
